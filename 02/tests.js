...
// part one tests:
function test_row_checksum(string, rv) {
  QUnit.test('row_checksum("' + string + '") ==== ' + rv, function(assert) {
    assert.equal(row_checksum(string), rv)
  })
}

function test_checksum(string, rv) {
  QUnit.test('checksum("' + string + '") ==== ' + rv, function(assert) {
    assert.equal(checksum(string), rv)
  })
}

test_row_checksum('5\t1\t9\t5', 8)
test_row_checksum('7\t5\t3', 4)
test_row_checksum('2\t4\t6\t8', 6)

test_checksum('5\t1\t9\t5', 8)
test_checksum('7\t5\t3', 4)
test_checksum('2\t4\t6\t8', 6)
test_checksum('5\t1\t9\t5\n7\t5\t3', 12)
test_checksum('5\t1\t9\t5\n7\t5\t3\n2\t4\t6\t8', 18)

// part one solution:
QUnit.test('checksum(puzzle_input) === 47623', function(assert) {
  assert.equal(checksum(puzzle_input), 47623)
})

// part two tests:
function test_row_checksum2(string, rv) {
  QUnit.test('row_checksum2("' + string + '") === ' + rv, function(assert) {
    assert.equal(row_checksum2(string), rv)
  })
}
function test_checksum2(string, rv) {
  QUnit.test('checksum2("' + string + '") ==== ' + rv, function(assert) {
    assert.equal(checksum2(string), rv)
  })
}

test_row_checksum2('5\t9\t2\t8', 4)
test_row_checksum2('9\t4\t7\t3', 3)
test_row_checksum2('3\t8\t6\t5', 2)
